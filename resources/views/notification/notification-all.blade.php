        ...
        <div class="p-4 mb-2 mx-2 rounded-lg border-2 {{ $row->is_read_user === 0 ? 'bg-red-50' : '' }}">
            <div class="w-[30%] flex items-center rounded-lg">
                ...
                @if($row->status === 'revisi')
                    <p class="font-semibold text-xs text-yellow-600 px-3 py-2 bg-yellow-50 rounded-lg">Laporan Revisi!</p>
                @elseif($row->status === 'tolak')
                    <p class="font-semibold text-xs text-red-600 px-3 py-2 bg-red-50 rounded-lg">Laporan Tolak!</p>
                @elseif($row->status === 'terbit')
                    <p class="font-semibold text-xs text-green-600 px-3 py-2 bg-green-50 rounded-lg">Laporan Diterima!</p>
                @endif
            </div>
