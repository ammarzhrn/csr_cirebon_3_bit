            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mb-6">
                    <div class="flex flex-wrap items-center space-x-2">
                        ...
                        <span class="font-semibold text-sm text-red-600">
                            {{ __('Edit Sektor dan Program') }}
                        </span>
                    </div>
                </div>

                <h2 class="text-2xl sm:text-[28px] font-semibold leading-tight sm:leading-[44px] tracking-[-0.02em] text-left font-inter mb-4">Edit Sektor dan Program</h2>

                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <form action="{{ route('dashboard.sektor.update', $sektor->id) }}" method="POST" enctype="multipart/form-data" id="updateForm">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="nama_sektor" id="nama-sektor-input" value="{{ $sektor->nama_sektor }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                            </div>

                            <!-- Deskripsi -->
                            <div class="mb-4">
                                <label for="deskripsi-input" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                                <textarea name="deskripsi" id="deskripsi-input" rows="3" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>{{ $sektor->deskripsi }}</textarea>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div id="programsContainer">
                                    @foreach($sektor->programs as $index => $program)
                                    <div class="program mb-6 flex flex-wrap items-start">
                                        ...
                                        <div class="w-full md:w-1/3 pr-2 mb-2 md:mb-0">
                                            <label for="nama-program-{{ $index }}" class="block text-sm font-medium text-gray-700">Nama Program</label>
                                            <input type="text" name="nama_program[]" id="nama-program-{{ $index }}" value="{{ $program->nama_program }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                                        </div>
                                        <div class="w-full md:w-3/5 px-2 mb-2 md:mb-0">
                                            <label for="deskripsi-program-{{ $index }}" class="block text-sm font-medium text-gray-700">Deskripsi Program</label>
                                            <textarea name="deskripsi_program[]" id="deskripsi-program-{{ $index }}" rows="4" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>{{ $program->deskripsi }}</textarea>
                                        </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-5">
                    <div class="bg-white border-b border-gray-200">
                        <div class="flex items-center justify-end px-6 py-4 border border-gray-200 rounded-lg">
                            <button type="submit" form="updateForm" class="flex items-center justify-center w-[119px] h-[52px] px-[18px] py-[10px] gap-2 rounded-lg border border-[#98100A] bg-[#98100A] text-white shadow-sm hover:bg-[#7d0d08] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#98100A]">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <span class="text-base font-semibold">Perbarui</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        // Jika sudah ada thumbnail, tampilkan
                        @if($sektor->thumbnail)
                        thumbnailPreview.style.backgroundImage = `url({{ asset('storage/' . $sektor->thumbnail) }})`;
                        thumbnailPreview.style.backgroundSize = 'cover';
                        thumbnailPreview.style.backgroundPosition = 'center';
                        thumbnailPreview.style.backgroundRepeat = 'no-repeat';
                        uploadPrompt.style.display = 'none';  // Hide upload prompt
                        fileTypeText.textContent = '{{ basename($sektor->thumbnail) }}';
                        fileTypeText.className = 'absolute top-2 left-2 text-sm text-white bg-black bg-opacity-50 px-2 py-1 rounded';
                        fileTypeText.style.display = 'block';  // Ensure file name is visible
                        @endif
                    });
