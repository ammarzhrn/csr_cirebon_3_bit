                ...
                <div class="bg-white shadow-md rounded-xl border border-gray-200 overflow-hidden">
                        ...
                        <div id="laporanTableContainer">
                            <table class="w-full table-auto">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200" id="laporanTableBody">
                                    ...
                                    @if(auth()->user()->level === 'admin')
                                    @if ($laporan->status === 'terbit' || $laporan->status === 'pending' || $laporan->status === 'tolak' || $laporan->status === 'revisi')
                                        ...
                                        <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}" data-status="{{ $laporan->status }}">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                Kec. {{ is_string($laporan->formatted_lokasi) ? $laporan->formatted_lokasi : json_encode($laporan->formatted_lokasi) }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {{ is_string($laporan->formatted_realisasi) ? $laporan->formatted_realisasi : json_encode($laporan->formatted_realisasi) }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {{ is_string($laporan->formatted_tgl_realisasi) ? $laporan->formatted_tgl_realisasi : json_encode($laporan->formatted_tgl_realisasi) }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {{ is_string($laporan->formatted_tgl_laporan) ? $laporan->formatted_tgl_laporan : json_encode($laporan->formatted_tgl_laporan) }}
                                            </td>
                    ...
                    <div class="md:hidden">
                        @foreach($laporans as $index => $laporan)
                            @if(auth()->user()->level === 'admin')
                                @if ($laporan->status === 'terbit' || $laporan->status === 'pending' || $laporan->status === 'tolak' || $laporan->status === 'revisi')
                                    <div class="p-4 border-b {{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}" data-status="{{ $laporan->status }}">
                                        <h3 class="font-semibold text-lg mb-2">{{ $laporan->judul_laporan }}</h3>
                                        <p class="text-sm mb-1"><span class="font-medium">Lokasi:</span> Kec. {{ is_string($laporan->formatted_lokasi) ? $laporan->formatted_lokasi : json_encode($laporan->formatted_lokasi) }}</p>
                                        <p class="text-sm mb-1"><span class="font-medium">Realisasi:</span> {{ is_string($laporan->formatted_realisasi) ? $laporan->formatted_realisasi : json_encode($laporan->formatted_realisasi) }}</p>
                                        <p class="text-sm mb-1"><span class="font-medium">Tgl Realisasi:</span> {{ is_string($laporan->formatted_tgl_realisasi) ? $laporan->formatted_tgl_realisasi : json_encode($laporan->formatted_tgl_realisasi) }}</p>
                                        <p class="text-sm mb-1"><span class="font-medium">Laporan Dikirim:</span> {{ is_string($laporan->formatted_tgl_laporan) ? $laporan->formatted_tgl_laporan : json_encode($laporan->formatted_tgl_laporan) }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getFilterParams() {
        return `tahun=${tahunFilter ? tahunFilter.value : ''}&quarter=${quarterFilter ? quarterFilter.value : ''}`;
    }

    // Tambahkan event listener ke tombol download CSV jika ada
    ...
    // Tambahkan event listener ke tombol download PDF jika ada
